...
void draw() {
  ...
  PImage aux2 = createImage(img.width, img.height, RGB);
   ...
   int jan = 4; 
    ...
    aux = realcarCor(aux,100); //Faz a saturação de todos os pixels proximo ao 0, para deixar mais evidente e facilitar a identificação
  ...
  image(img,0,0);
  image(imgMask,0,imgMask.height + 10);
...
PImage limiarizar(PImage aux2){    
   ...
   for (int y = 0; y < aux2.height; y++) {
    for (int x = 0; x < aux2.width; x++) {
     ...
     if((blue(aux2.pixels[pos]) > 50) && ((x >280 && x <320) && (y >200 && y < 220)||(x >220 && x <241) && (y >20 && y < 40)))aux2.pixels[pos] =color(0);    
     if(((x >228 && x <240) && (y >52 && y < 65)))aux2.pixels[pos] =color(255);     
    }
